...
public class FoodDAO  extends CustomSqliteHelper {
	...
	public ArrayList<Food> getByShopAndCategory(int shopId, String category){
		ArrayList<Food> foods = new ArrayList<Food>();
		Cursor c = this.getWritableDatabase().query(TABLE_NAME, null, "shopId = ? and category = ?", 
				new String[]{String.valueOf(shopId), category}, null, null, "o ASC", "1");
		while (c.moveToNext()){
			int foodId = c.getInt(c.getColumnIndex(FIELD_ID));
			String cat = c.getString(c.getColumnIndex(FIELD_CATEGORY));
			String name = c.getString(c.getColumnIndex(FIELD_NAME));
